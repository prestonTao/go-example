/*
    ...
    如果p 是一个素数，g 和x 是整数，计算 y = gx mod p 非常快。但是相反的过程：先知道 p, g 和y，要求某个x(离散对数)，
    满足等式y = gx modp，通常十分困难。此相反求离散对数的过程称为“离散对数问题”。例如：如果15 = 3xmod17, 则x = 6。
    g和p的选择对此类系统的安全性影响很大。为了保证无法求解离散对数问题，p应该是一个很大的素数，例如1024bit的，而且
    (p-1)/2也应该是素数。G要求是p的primitive root，也就是讲整数序列： g mod p,g mod p, g2mod p, …., gp-1 modp中的p个
    元素均是不同的整数。
